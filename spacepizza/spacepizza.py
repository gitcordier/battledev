...
#
#   ii. If we are at p, we can reach any p' such that 
#               y(p') > y(p) .
#       But points with smaller are out of reach.
#
#   iii.We so reach p_max, the point with the greated y.
#
# B. Return back ("as y decreases"):
#   i.  We start from p_max, the point with the greatest y.
#
#   ii. If we are at p, we can reach any p" such that 
#               y(p") < y(p) .
#       But points with greater y are out of reach.
#
#   iii.We MUST visit every p' that was not visited outward
#       (so that all points get visited, eventually);
#
#   iv. No point can be visited twice.
# 
#   v. Finally, we come back to p_min. So ends the journey :)
#
# It seems there are many possible trips, then (actually, yes: There 
# are exactly 2^(N-2) possible trips)!
# 
# So arises the question: What is the shortest one?
#
# This is what we aim at computing now. 
#
# To do so, we first sort the points by increasing y, so that 
# the y of node #j is smaller than the y of node #i (j < i).
# 
# Next, we split the set of all possible paths into subsets S[i]:
#
#       S[i] := {    
#                   All trips during which the point #i is 
#                   visited just before we 'jump' to the 
#                   'highest point, i.e. the point #(N-1). 
#               }
#                   (i = 0, 1, 2, ..., N-2) .
#
# Given i,there so exits a minimal distance for the whole trip, 
# namely:
#
...
#
#       min___total := min{min_i_total: i = 0, 1, 2, ..., N-1} .
#
...
#           |       |i+1|
#           |        ...
#           |        N-2  <--| Distances are fixed, provided i.
#           N-1      N-1 
#
# NB:
...
distance = [[get_distance_between(node_[j], node_[i]) for j in range(i)]
                for i in range(0, N)]

def d(j, i):
    """
        Since points are indexed, we find more customary to deal with 
        d(j, i) instead of d(point #j, d(point #i) (i < j). 
        Bear in mind that j MUST be smaller than i. 
    """
    return distance[i][j]
...
last_jump_from = [d(i, N-1) for i in range(N-2)]
...
# i = 0
    min_[0]     = d(0, 1)
    min_0_total = min_[0] + last_jump_from[0] + s[1]

# i = 1
    min_[1]     = min_[0] + length(1)
    min_1_total = min_[1] + last_jump_from[1] + s[2]

# So, for now,
    min___total = min(  min_0_total, 
                        min_1_total)
...
# i = 2, 3, ..., N-3 (N > 4)
    for i in range(2, N-2): 
        ...
        #
        min_[i]     = min_i
        min_i_total = min_[i] + last_jump_from[i] + s[i+1]
        min___total = min(  min___total, 
                            min_i_total)
    return min___total
#
